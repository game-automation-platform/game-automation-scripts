...
Tsum.prototype.dumpUiXml = function() {
  ...
  if (typeof xml !== 'string' || xml.indexOf('<hierarchy') === -1) {
    ...
    logWarn(Log.Dialog.UiDumpUnavailable, 'uiautomator dump unavailable; using pixels only',
      { failures: this._uiDumpFailures, maxFailures: 2 });
    return '';
  }
...
Tsum.prototype.tapDialogButtonViaUi = function(box) {
  ...
  logInfo(Log.Dialog.TapViaUi, 'Tapping a dialog button found by uiautomator',
    { label: target.text, x: target.x, y: target.y });
...
Tsum.prototype.tryDismissDialog = function(box, hint) {
  ...
  if (btn !== null) {
    logInfo(Log.Dialog.TapRightmostLabel, 'Tapping the rightmost label in the dialog button row',
      { x: btn.x, y: btn.y });
    tap(btn.x, btn.y, 60);
    ...
    logWarn(Log.Dialog.NoButtonRow, 'No button row found in the dialog panel');
  }
  if (this.tapDialogButtonViaUi(box) && this.dialogChanged(box)) {
    return true;
  }
  if (hint !== undefined && hint !== null) {
    logInfo(Log.Dialog.TapPageHint, 'Trying the coordinates recorded for the matched page variant',
      { x: hint.x, y: hint.y });
    this.tap(hint);
    if (this.dialogChanged(box)) {
      return true;
  ...
  if (this.autoLaunch) {
    ...
    logWarn(Log.Dialog.DpadFallback, 'Falling back to dpad focus and enter');
    for (let i = 0; i < 5; i++) {
  ...
  logError(Log.Dialog.DismissFailed,
    'Could not dismiss the dialog -- send the saved screenshot if this keeps happening');
...
Tsum.prototype.dismissSystemDialog = function(hint) {
  ...
  for (let round = 0; round < 3 && this.isRunning; round++) {
    ...
    logInfo(Log.Dialog.Detected, 'Native dialog on screen', {
      x: box.dx0,
      y: box.dy0,
      width: box.dx1 - box.dx0,
      height: box.dy1 - box.dy0,
      units: 'devicePx',
    });
...
Tsum.prototype.saveDebugScreenshot = function(tag) {
  ...
  try {
    saveImage(img, path);
    logInfo(Log.Dialog.ScreenSaved, 'Saved a debug screenshot', { path: path });
  } catch (e) {
    logWarn(Log.Dialog.ScreenSaveFailed, 'Could not save the debug screenshot',
...
// Stall guard
//
// `gPages.navigate()` only exits by reaching its destination, so one screen it
// cannot leave wedged the whole script: taskPlayGameQuick never returns, which
// means the task controller never gets to run anything else either -- including
// the app restart task. The navigation loop reports the page it just handled;
// when the screen stops moving, escalate: look for a modal dialog first, then
// restart the app.
//
// This is the backstop for the whole subscription queue, not just for one loop:
// a page with no mover that applies, a mover whose tap misses, and a screen
// that fingerprints as two different things on alternate polls all look the
// same from here -- nothing is moving.
// ---------------------------------------------------------------------------
...
Tsum.prototype.newStallGuard = function(where) {
  return {where: where, page: '', repeats: 0, rounds: 0, dialogTried: false, restarts: 0, noRoute: ''};
};
...
Tsum.prototype.checkStall = function(guard, page) {
  ...
  if (!guard.dialogTried && stuck >= StallDialogAfter) {
    ...
    logWarn(Log.Stall.CheckingDialog, 'Stuck on a page; checking for a native dialog',
      { where: guard.where, page: page });
  ...
  if (guard.restarts >= StallMaxRestarts || !this.autoLaunch) {
    logError(Log.Stall.GivingUp, this.autoLaunch
      ? 'Still stuck after restarting the game; waiting before trying again'
      : 'Still stuck -- enable "Auto launch app" to let the script restart the game', {
      where: guard.where,
      page: page,
      restarts: guard.restarts,
      maxRestarts: StallMaxRestarts,
      autoLaunch: this.autoLaunch,
      waitMs: 30000,
    });
    this.saveDebugScreenshot('stall');
    // A screen the loop cannot leave is worth keeping whether or not the table
    // recognised it: either the fingerprint is wrong, or the handling is.
    this.saveCorpusFrame('stall');
    this.sleep(30000);
    return;
  }
  guard.restarts++;
  logWarn(Log.Stall.RestartingApp, 'Stuck on a page; restarting the game app',
    { where: guard.where, page: page, restarts: guard.restarts, maxRestarts: StallMaxRestarts });
