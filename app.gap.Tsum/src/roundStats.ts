...
// Digit shapes, normalised to StatsGlyphW x StatsGlyphH. Every sample of a
// digit found in the reference screenshots classifies against these with at
// least a 0.036 lead over the runner-up, so they can be re-cut from a new
// screenshot if the game ever changes its typeface -- and '5' has been, from
// the corpus `five-score` / `five-coins` tallies. The original '5', with its
// three-row top bar, sat a 0.02 margin from the game's real glyph: the game
// draws '5' with a one-row bar and a long left stem, which is most of a '6',
// and the small coin-row '5's were flipping to '6' outright. The one shape
// that still separates them is rows 8-9, where '5' is empty on the left and
// '6' keeps its stroke. `pages:stats` measures all of this over the corpus.
